...
switch ($action) {
    ...
    case 'login':
		...
		try {
			...
			// ── Rate limiting logowania (3 próby / minutę per IP) ──
			...
			$rateLimitFile = $rateLimitDir . 'login_' . md5($ip) . '.json';
			$rateLimitMax = 3;        // max prób
			$rateLimitWindow = 60;    // per 60 sekund
			$rateLimitLockout = 300;  // blokada na 5 minut po przekroczeniu

			$rateData = array('attempts' => array(), 'lockout_until' => 0);
			if (file_exists($rateLimitFile)) {
				$raw = @file_get_contents($rateLimitFile);
				if ($raw) {
					$parsed = json_decode($raw, true);
					if (is_array($parsed)) {
						$rateData = array_merge($rateData, $parsed);
					}
				}
			}

			// Sprawdź lockout
			if (isset($rateData['lockout_until']) && $rateData['lockout_until'] > time()) {
				$wait = $rateData['lockout_until'] - time();
				jsonError('Zbyt wiele prób logowania. Spróbuj za ' . $wait . ' sekund.', 429);
			}

			// Wyczyść stare próby (poza oknem)
			$now = time();
			$rateData['attempts'] = array_values(array_filter($rateData['attempts'], function($t) use ($now, $rateLimitWindow) {
				return ($now - $t) < $rateLimitWindow;
			}));

			// Sprawdź limit
			if (count($rateData['attempts']) >= $rateLimitMax) {
				$rateData['lockout_until'] = $now + $rateLimitLockout;
				@file_put_contents($rateLimitFile, json_encode($rateData), LOCK_EX);
				try {
					logAction(0, 'login_blocked', 'security', 0, "IP zablokowane po " . $rateLimitMax . " próbach: {$ip}");
				} catch (Exception $e) {}
				jsonError('Zbyt wiele prób logowania. Spróbuj za 5 minut.', 429);
			}
			...
			if (!$user) {
				...
				// Constant-time — nie ujawniaj czy user istnieje
			...
			$stmt->execute([
				...
				':ua'       => substr(isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '', 0, 500),
				':expires'  => $expiresAt
			]);
			...
			// Udane logowanie — wyczyść rate limit
			if (file_exists($rateLimitFile)) {
